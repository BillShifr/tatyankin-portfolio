...
const Layout = () => {
  ...
  return (
    ...
    >
      ...
      <ScrollBlockModal
        title="Секундочку, позволь словить твой фокус"
        content={`Важный момент: здесь речь не о демонстрации технических навыков, а о накопившейся боли и непонимании во взаимодействии соискателя и работодателя.\n\nВ ИТ я пришёл далеко не случайно, а целенаправленно, шаг за шагом. Всё началось с первых кубиков конструктора, продолжилось любовью к техническим наукам с начальных классов, затем — лицей ЮФМЛ, и, наконец, высшее образование по специальности. За этот путь удалось попробовать множество технологий и подходов, так что техническая база уверенная. Всё-таки, я из тех, для кого в 1 КБ — 1024 байта :)\n\nСвоей сильнейшей стороной считаю креативность и генерацию идей. Яркое достижение вне работы — открытие (и что ключевое — **без бюджета**) химчистки обуви в нашем городе. Она успешно работает уже 6 лет и пережила нескольких конкурентов. Кстати, сама идея такого бизнеса тоже была сгенерирована самостоятельно.\n\n**А ТЕПЕРЬ К СУТИ.**\n\nУже несколько месяцев нахожусь в активном поиске работы. Если вы читаете это — значит, он всё ещё продолжается.\n\nЗа это время я проделал огромную работу: изучил механизмы HeadHunter, оптимизировал сотни сопроводительных писем, переработал опыт и навыки в профиле. И всё, казалось бы, впустую.\nДело дошло до того, что пришлось создать собственного бота на базе API HH с индивидуальной логикой подбора вакансий. Он не просто спамит откликами, а анализирует текст вакансии, сравнивает с моим профилем и решает, стоит ли тратить ваше и моё время. Жаль, что на той стороне рекрутинга редко встречается такая же аналитика.\nНо даже так, процент немотивированных отказов стабильно продолжает держаться на уровне 8-9 из 10. 🔥\n\n**В чём проблема? У меня есть на это ответ. И есть конкретное решение.** В этой ситуации теряют обе стороны — и работодатель, и соискатель. Детали я готов рассказать при личной встрече 1:1 или 1:? главное с вашей командой :)\n\n**Я ХОЧУ РАБОТАТЬ. Я УМЕЮ РАБОТАТЬ. И ГОТОВ ЭТО ДОКАЗАТЬ.** Но не в роли специалиста средней руки (этот путь, как путь юнги до офицера, я уже прошёл), а на позиции, где у меня будет больше пространства для манёвра, идей и принятия решений — в управленческой или технической роли с лидерской составляющей.`}
      />
    </div>
  )
}
